...
-- information for a specific FCC BOQ.
-- =============================================
CREATE PROC [dbo].[spGetBOQFCCIndicatorValues] @BOQFCCPK INT = NULL
AS
BEGIN
    ...
    --Get the indicator information only for V2 forms
    SELECT pivotedTable.BenchmarkOfQualityFCCPK,
    ...
    (
        ...
        WHERE boqfcc.BenchmarkOfQualityFCCPK = @BOQFCCPK
    ) boqfcc
        UNPIVOT
        (
            IndicatorValue
            FOR Indicator IN (Indicator1, Indicator2, Indicator3, Indicator4, Indicator5, Indicator6, Indicator7,
                              ...
                              Indicator27, Indicator28, Indicator29, Indicator30, Indicator31
                             )
        ) AS pivotedTable
	...
	INNER JOIN dbo.CodeBOQCriticalElement cbce
		ON cbce.CodeBOQCriticalElementPK = cbi.BOQCriticalElementCodeFK
			AND cbce.BOQTypeCodeFK = 5 --Only want BOQ FCC V2 critical elements and indicators
	INNER JOIN dbo.CodeBOQIndicatorValue cbiv
		ON cbiv.IndicatorValue = pivotedTable.IndicatorValue
			AND cbiv.BOQTypeCodeFK = 5;  --Only want BOQ FCC V2 indicator values
